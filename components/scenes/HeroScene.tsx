  ...
  return (
    ...
    >
      <ScrollReveal className="text-center">
        <h1 className="font-body text-[2.05rem] leading-[1.3] text-ivory sm:text-[2.6rem]">
          {scene.content.title}
          {scene.content.subtitle ? <span className="block">{scene.content.subtitle}</span> : null}
          <span className="block">{scene.content.body}</span>
        </h1>
        <p className="mt-7 font-heading text-[0.64rem] uppercase tracking-[0.22em] text-ivory/95 sm:text-xs">{scene.content.accent}</p>
        {scene.content.secondaryBody ? (
          <p className="mt-1.5 font-heading text-[0.64rem] uppercase tracking-[0.22em] text-ivory/95 sm:text-xs">
            {scene.content.secondaryBody}
          </p>
        ) : null}
        <div className="mx-auto mt-10 h-16 w-px bg-ivory/75" aria-hidden />
